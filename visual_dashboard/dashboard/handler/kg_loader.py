"""
Knowledge Graph data loader and subgraph extraction for the dashboard.
...
def load_kg_data():
    ...
    # Build a list of (node_id, attr_dict) tuples using to_dict('records')
    attr_records = entities_df[attr_cols].to_dict("records")
    # Drop NaN values from each attr dict
    nodes_with_attrs = [
    ...
    # Filter to edges where both endpoints exist (should be all, but safe)
...
def extract_subgraph(G, center_node_id, hops=1, entity_type_filter=None):
    ...
    if hops == 1:
        ...
        # Smart 2-hop: get hop-1 entities, then sample hop-2 incidents per entity
        hop1_nodes = set(nx.ego_graph(undirected, center_node_id, radius=1).nodes())
        hop1_entities = hop1_nodes - {center_node_id}

        neighborhood_nodes = set(hop1_nodes)  # start with all hop-1

        for entity_id in hop1_entities:
            # Get other incidents connected to this entity (hop-2 incidents)
            other_incidents = []
            ...
            # For each sampled incident, also include its direct entities
            # so we can see what they have in common
            for inc in sampled:
